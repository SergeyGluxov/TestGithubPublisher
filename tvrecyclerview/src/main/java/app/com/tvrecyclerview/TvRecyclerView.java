...
public class TvRecyclerView extends RecyclerView {
    ...
    private View mNextFocusedDown;
    private View mNextFocusedUp;
    private View mNextFocusedRight;
    private View mNextFocusedLeft;
    ...
    public static final long LONG_INTERVAL_FOCUS_CHANGE = 300;
    //Скорость фокусировки на следующий элемент списка в миллисекундах
    public static final long SHORT_INTERVAL_FOCUS_CHANGE = 150;
    private long lastTimeFocusSearch = 0L;
    private long intervalFocusChange = LONG_INTERVAL_FOCUS_CHANGE;
    private boolean isDisableDownLimit = false;
    private View mLastFocusedView;
    private View mNextFocusFinder;
    ...
    /* @Override
     protected boolean onRequestFocusInDescendants(int direction, Rect previouslyFocusedRect) {
         ...
         int fvPos = 0;
         int lvPos = 0;
         int delta = lvPos - fvPos;
         int findByPos = fvPos + delta / 2;
         View view = null;
         for (int i = findByPos; i < findByPos + delta / 2; i++) {
         ...
         if (view != null) {
             // TODO: 29.05.20 view.isFocusable=false может выстрелить
 ...
 */
    private void setAttributeSet(AttributeSet attrs) {
        if (attrs != null) {
            ...
            intervalFocusChange = (long) typeArray.getInteger(R.styleable.TvRecyclerView_intervalFocus, 150);
            if (!mIsAutoProcessFocus) {
    ...
    public int getFirstVisiblePosition() {
        ...
        Log.d(TAG, "getFirstVisiblePosition: firstVisiblePos==" + firstVisiblePos);
    ...
    public void setLayoutManager(LayoutManager layoutManager) {
        ...
        Log.d(TAG, "setLayoutManager: orientation==" + mOrientation);
        super.setLayoutManager(layoutManager);
    }

    public void disableDownFocusLimit() {
        isDisableDownLimit = true;
    }

    public void setIntervalFocusChange(long interval) {
        Log.d(TAG, "setIntervalFocusChange: interval==" + interval);
        intervalFocusChange = interval;
    }

    private View findNextFocus(View focused, int direction) {
        Log.d(TAG, "findNextFocus: focused==" + focused.getId() + " direction== " + direction);

        final long current = System.currentTimeMillis();
        if (current - lastTimeFocusSearch < intervalFocusChange) {
            return focused;
        }
        lastTimeFocusSearch = current;
        final View nextFocus = super.focusSearch(focused, direction);
        if (isDisableDownLimit) {
            return nextFocus;
        } else {
            switch (direction) {
                case View.FOCUS_UP:
                case View.FOCUS_DOWN:
                    if (nextFocus != null && focused != null && nextFocus.getParent() != focused.getParent()) {
                        return focused;
                    } else {
                        return nextFocus;
                    }
    ...
    public View focusSearch(View focused, int direction) {
        Log.d(TAG, "focusSearch: focused==" + focused.getId() + " direction== " + direction);
        mDirection = direction;
        if (focused == this) {
            return getLastFocusedView();
        }
        mLastFocusedView = findNextFocus(focused, direction);
        if ((mDirection == View.FOCUS_LEFT || mDirection == View.FOCUS_RIGHT)
                && mLastFocusedView != null) {
            if (mLastFocusedView.getParent() instanceof TvRecyclerView) {
                final TvRecyclerView tvRecyclerView = (TvRecyclerView) mLastFocusedView.getParent();
                if (tvRecyclerView != this && tvRecyclerView.getLastFocusedView() != null) {
                    if (!tvRecyclerView.isFocusable()) {
                        return null;
                    }
                    return tvRecyclerView.getLastFocusedView();
                }
            }
            if (mNextFocusFinder != null) {
                return ((ViewParent) mNextFocusFinder).focusSearch(mNextFocusFinder, direction);
            }
        }
        return mLastFocusedView;
    }

    public void setNextFocusFinder(View nextFocusFinder) {
        mNextFocusFinder = nextFocusFinder;
    }

    public void setNextFocusUp(View nextFocusedUp) {
        mNextFocusedUp = nextFocusedUp;
    }

    public void setNextFocusDown(View nextFocusedDown) {
        mNextFocusedDown = nextFocusedDown;
    }

    public void setNextFocusRight(View nextFocusedRight) {
        mNextFocusedRight = nextFocusedRight;
    }

    public void setNextFocusLeft(View nextFocusedLeft) {
        mNextFocusedLeft = nextFocusedLeft;
    }
    ...
    public void setItemSelected(int position) {
        Log.d(TAG, "setItemSelected: position==" + position);

        if (mSelectedPosition == position || getAdapter() == null) {
            return;
        }
    ...
    public View getLastFocusedView() {
        return getLayoutManager().findViewByPosition(mSelectedPosition);
    }
    ...
    public void requestChildFocus(View child, View focused) {
        ...
        if (onFocusChangeListener != null) {
            onFocusChangeListener.onFocusChange(child, child.hasFocus());
        }
    ...
    public void alignFocusedItem() {
        Log.d(TAG, "alignFocusedItem: mLastFocusedView== " + mLastFocusedView.getId());

        if (mLastFocusedView != null) {
            scrollView(mLastFocusedView);
        }
    }

    private void scrollView(View focused) {
        Log.d(TAG, "scrollView: focused== " + focused.getId());
    ...
    private int getAdapterPositionByView(View view) {
        Log.d(TAG, "getAdapterPositionByView: view== " + view.getId());
...
//    }
    ...
    protected void onRestoreInstanceState(Parcelable state) {
        ...
        Log.d(TAG, "onRestoreInstanceState: select_pos== " + bundle.getInt("select_pos", 0));
    ...
    protected Parcelable onSaveInstanceState() {
        ...
        Log.d(TAG, "onSaveInstanceState: select_pos== " + bundle.getInt("select_pos", 0));
    ...
    private void updateSelectPositionInLayout(View nextView) {
        Log.d(TAG, "updateSelectPositionInLayout nextView== " + nextView.getId());
    ...
    private void scrollToViewToCenter(View view) {
        Log.d(TAG, "scrollToViewToCenter view== " + view.getId());
    ...
    private void scrollToView(View view, boolean smooth) {
        Log.d(TAG, "scrollToViewToCenter view== " + view.getId() + " smooth== " + smooth);
    ...
    private int getNeedScrollDistance(View focusView) {
        Log.d(TAG, "getNeedScrollDistance focusView== " + focusView.getId());
    ...
    private int getAlignDistance(View view) {
        ...
        Log.d(TAG, "getAlignDistance view== " + view.getId() + " scrollDistance " + scrollDistance);
    ...
    private int getFollowDistance(View view) {
        ...
        Log.d(TAG, "getFollowDistance view== " + view.getId() + " scrollDistance== " + scrollDistance);
    ...
    private int getNormalDistance(View view) {
        ...
        Log.d(TAG, "getNormalDistance view== " + view.getId() + " scrollDistance== " + scrollDistance);
    ...
    private int getScrollPrimary(View view) {
        ...
        Log.d(TAG, "getScrollPrimary view== " + view.getId() + " distance== " + distance);
    ...
    private void smoothScrollView(int scrollDistance) {
        if (mOrientation == HORIZONTAL) {
            ...
            smoothScrollBy(0, scrollDistance, mLinearInterpolator);
        }
        Log.d(TAG, "smoothScrollView  scrollDistance == " + scrollDistance);
    ...
    private int getNormalScrollDistance(View view) {
        ...
        Log.d(TAG, "getNormalScrollDistance  view == " + view.getId() + " distance== " + distance);
    ...
    private boolean isHalfVisibleChild(View child) {
        ...
        if (child != null) {
            Log.d(TAG, "isHalfVisibleChild  child == " + child.getId());
    ...
    private boolean isVisibleChild(View child) {
        if (child != null) {
            Log.d(TAG, "isVisibleChild  child == " + child.getId());
    ...
    private boolean isOverHalfScreen(View child) {
        Log.d(TAG, "isOverHalfScreen  child == " + child.getId());
    ...
    private final class PendingMoveSmoothScroller extends LinearSmoothScroller {
        ...
        private int mMaxPendingMoves = 1;
